...
use crate::{routes, models::{BoxRecord, now_str}};
...
async fn test_get_boxes_empty_for_new_user() {
    ...
    let response = app
        .oneshot(create_request("GET", "/boxes", "new_user", None))
        .await
        .unwrap();
...
async fn test_get_box_not_found() {
    ...
    let initial_boxes_response = app.clone()
        .oneshot(create_request(
            "GET",
            "/boxes",
            "user_1",
    ...
    let response = app.clone()
        .oneshot(create_request(
            "GET",
            &format!("/boxes/{}", non_existent_box_id),
            "user_1",
            None
        ))
        .await
...
async fn test_update_box() {
    ...
    let initial_data = response_to_json(initial_response).await;
    let _initial_name = initial_data["box"]["name"].as_str().unwrap().to_string();
    let _initial_description = initial_data["box"]["description"].as_str().unwrap().to_string();
...
async fn test_update_box_not_owned() {
    ...
    // Verify nothing has changed
    assert_eq!(final_name, initial_data["box"]["name"].as_str().unwrap());
    assert_eq!(final_description, initial_data["box"]["description"].as_str().unwrap());
}
...
async fn test_delete_box() {
    ...
    // 2. Delete the box
    let delete_response = app2.clone()
        .oneshot(create_request(
            "DELETE",
            &format!("/boxes/{}", box_id),
            "owner_user",
