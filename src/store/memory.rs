...
use crate::models::{BoxRecord, now_str};
use super::BoxStore;
...
impl BoxStore for MemoryBoxStore {
    async fn create_box(&self, box_record: BoxRecord) -> Result<BoxRecord> {
        let mut boxes = self.boxes.write().map_err(|_| 
            AppError::InternalServerError("Failed to acquire write lock".into()))?;
            
        if boxes.contains_key(&box_record.id) {
            return Err(AppError::BadRequest(format!("Box with ID {} already exists", box_record.id)));
        }
    ...
    async fn get_box(&self, id: &str) -> Result<BoxRecord> {
        let boxes = self.boxes.read().map_err(|_| 
            AppError::InternalServerError("Failed to acquire read lock".into()))?;
    ...
    async fn get_boxes_by_owner(&self, owner_id: &str) -> Result<Vec<BoxRecord>> {
        ...
        let owner_boxes: Vec<BoxRecord> = boxes.values()
    ...
    async fn update_box(&self, box_record: BoxRecord) -> Result<BoxRecord> {
        ...
        if !boxes.contains_key(&box_record.id) {
            return Err(AppError::NotFound(format!("Box not found: {}", box_record.id)));
        }
...
/// Type alias for the Arc-wrapped Memory store
pub type MemoryStore = Arc<MemoryBoxStore>;